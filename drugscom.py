    ...
    def __init__(self):
        ...
        self.color_codes = [{'id': 0, 'name': 'Beige', 'code': 14}, {'id': 1, 'name': 'Black', 'code': 73},
            ...
            {'id': 38, 'name': 'Grey & Peach'}, {
                'id': 39, 'name': 'Grey & Pink'}, {'id': 40, 'name': 'Grey & Red'},
        ...
        for i in range(3):
          ...
          # test every possible starting word (don't know where left/right break is)
                ...
                for _ in range(len(m2s) - 1):
                    ...
                    if m1ss == "".join(m2sq):
                        if not self.color_shape_match(target_color,color,target_shape, shape):
                            print('none match color/shape')
                        else:
                            return True
                    if j == 1:
...
#         elem = self.wait.until(EC.element_to_be_clickable((By.XPATH, "//a[@href='/imprints.php']")))
    ...
    #             print('a', a, type(a), a.text)
                mprint = a.text
                # remove repeated internal spacesBeautifulSoup
                mprint = ' '.join(mprint.split())
                print('mprint', mprint)
                if not self.mprint_is_equal(mprint, pmprint, target_color, color, target_shape, shape):
